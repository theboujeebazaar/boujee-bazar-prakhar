...
// Shiprocket → Settings → API → Configure Webhooks, along with a "Token" —
// set that exact same string as SHIPROCKET_WEBHOOK_SECRET below. Shiprocket
// sends that token back on every webhook call in the `x-api-key` header
// (the "Auth Token Type" you pick in their webhook form), which is how we
// verify the request actually came from Shiprocket and not a random POST
// to this public URL.
//
// NOTE: this route intentionally isn't named ".../shiprocket" — Shiprocket's
// own webhook setup form rejects URLs containing "shiprocket", "kartrocket",
// "sr", or "kr".
...
export async function POST(req: Request) {
  try {
    ...
    // `sr_order_id` (if present) is Shiprocket's own internal numeric id —
    // not used for matching here, but logged for reference.
    ...
    const courierName: string | undefined = payload.courier_name || undefined
    const currentStatus: string | undefined = payload.current_status || payload.shipment_status

    console.log('[Shiprocket Webhook]: Received update', {
      internalOrderId,
